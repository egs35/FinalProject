...
def load_reviews_from_csv(review_file):
    reviews = []
    with open(review_file, newline="") as csvfile:
...
for name, data in combined_restaurants.items():
    print(
        f"{name}: Ratings: {data['ratings']}, Reviews: {data['reviews']}, Prices: {data['prices']}, Locations: {data['locations']}"
    )
    print()
